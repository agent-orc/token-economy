...
- `run` constructs `ModelPrefixBenchmarkInvoker`. Models whose ids start with
  `claude-` are sent through the authenticated `claude` CLI; all other
  controlled variants are sent through `codex`.
