...
public sealed class CardEconomicsTests
{
    ...
    private static AgentStudioRunRecord Run(string task, int run, bool success, string model = "gpt-6-astra", string level = "low")
    {
        var json = JsonSerializer.Serialize(new
        {
            ...
            attempts = new[] { new { run, model, thinkingLevel = level, finalLane = success ? "done" : "failed",
                startedAt = Day.AddHours(1), completedAt = Day.AddHours(1).AddMinutes(1),
    ...
    {
        var q = Query() with { Card = new() { TaskKey = "new", TaskType = "feature", Prompt = "Fix fencing for lease ownership" } };
        var result = new CardEconomics().Decide(q, [Run("a", 1, true), Run("b", 1, true, "gpt-5.6-sol", "xhigh")]);
        Assert.False(result.Rows.Single(r => r.Candidate.Model == "gpt-6-astra").Eligible);
        Assert.True(result.Rows.Single(r => r.Candidate.Model == "gpt-5.6-sol").Eligible);
        Assert.Contains("gpt-5.6-sol/xhigh", result.Recommendation);
    }
