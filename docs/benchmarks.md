...
This survey uses the SWE-bench Original dataset/evaluation protocol and Aider's polyglot harness plus Exercism-derived corpus as its concrete anchors. It covers the properties transferable to controlled experiments on private repositories: task input, execution isolation, success criteria, and reported metrics. It deliberately does not import public benchmark instances or their leaderboards; Token Economy uses the established evaluation shapes while keeping prompts, source, and results local.

| Suite | Task format | Success criteria | Reported metrics | Transfer to Token Economy |
| --- | --- | --- | --- | --- |
| [SWE-bench Original](https://www.swebench.com/original.html) | Natural-language GitHub issue, repository, and fixed `base_commit`; the dataset also identifies a gold patch and test patch. | Candidate patch is evaluated with `FAIL_TO_PASS` tests for the repair and `PASS_TO_PASS` tests for regressions. | Instance resolution rate. | Isolated fixed workspace, issue-like prompt, executable gate, and binary resolved-instance quality. The official [dataset](https://www.swebench.com/SWE-bench/guides/datasets/) and [evaluation](https://www.swebench.com/SWE-bench/guides/evaluation/) guides define the fields and protocol used here. |
| [Aider polyglot harness](https://github.com/Aider-AI/aider/blob/main/benchmark/README.md) and [corpus](https://github.com/Aider-AI/polyglot-benchmark) | Compact Exercism implementation exercise, language-native files and tests, model plus edit-format configuration, executed in a disposable container. | An exercise succeeds only when all native tests pass; the harness can make multiple attempts. | Pass rate by attempt, well-formed edits, errors, time per case, and total cost. | Small deterministic exercises, native test commands, identical tasks across model configurations, and quality/resource reporting. The [leaderboard methodology](https://aider.chat/docs/leaderboards/) provides published comparisons. |
...
It deliberately differs in four ways, also recorded in the setup JSON:

1. It uses a versioned synthetic C# fixture tree rather than SWE-bench's public issue/PR plus `base_commit` or an Aider Exercism item, so it is redistributable, fast, and transferable to private-codebase benchmarking.
2. It requests one replacement file in a single turn rather than SWE-bench's repository patch or Aider's edit-format and test-feedback retry loop, so the first evidence run isolates model/thinking choice from tool orchestration. Agentic setups can omit `task.responseFile`.
...
4. It isolates cases with fresh temporary workspace copies but does not execute generated code in Aider's Docker boundary. The checked-in fixture and evaluation command are trusted project inputs; setups that evaluate untrusted code require an external sandbox before they are safe to run.
