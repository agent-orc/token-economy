...
if (args.Length == 3 && args[0] == "decide")
{
    ...
    var decision = new CardEconomics().Decide(query, store.Records);
...
if (args.Length != 2 || (args[0] != "run" && args[0] != "document-to-text" && args[0] != "aggregate"))
{
    ...
    Console.Error.WriteLine("  dotnet run --project src/TokenEconomy.Benchmarks -- decide <query.json> <agent-studio-task-storage>");
