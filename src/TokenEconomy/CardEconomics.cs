...
public sealed record CardEconomicsRow
{
    ...
    public IReadOnlyList<CohortAvailabilityFact> OrganisationEvents { get; init; } = [];
...
{
    public CohortEconomicsEvidence? CohortEvidence { get; init; }
}
...
public sealed class CardEconomics(ModelPriceCatalog? prices = null, ModelRoutingKnowledgeBase? knowledge = null)
{
    ...
    /// <summary>Keep unknown-level model observations alongside (never inside) level-specific estimates.</summary>
    public CardEconomicsDecision Decide(CardEconomicsQuery query, AgentStudioCohort cohort)
    {
        if (!Same(query.OrganisationId, cohort.Organisation))
            throw new ArgumentException("Cohort organisation must match the query.", nameof(query));
        var evidence = new AgentStudioCohortImporter(_prices).Summarize(cohort, query.SinceUtc, query.AsOfUtc);
        var decision = Decide(query, cohort.Records);
        var rows = decision.Rows.Select(row =>
        {
            var facts = cohort.Availability.Where(f => Same(f.Model, row.Candidate.Model)).ToArray();
            var exclude = query.ExcludeObservedAccessRefusals && facts.Any(f => f.EventCount > 0);
            return row with
            {
                OrganisationEvents = facts,
                Availability = facts.Length > 0 ? "partly usable; access refusal events observed; run failure rate unknown" : row.Availability,
                Eligible = row.Eligible && !exclude,
                EligibilityReason = exclude ? "Observed organisation access refusal; excluded by query; event-to-run attribution unknown." : row.EligibilityReason,
            };
        }).OrderByDescending(r => r.Eligible).ThenBy(r => r.Rank).Select((r, i) => r with { Rank = i + 1 }).ToArray();
        return decision with { Rows = rows, CohortEvidence = evidence };
    }
