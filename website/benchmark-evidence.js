window.tokenEconomyEvidence = (() => {
  ...
  function context(row) {
    ...
    const facts = [row.evidenceStatus === 'provisional' ? 'Local routing: provisional' : null, kinds[c.sourceKind] || c.sourceKind, c.sourcePublisher, c.harness ? `Harness: ${c.harness}${c.harnessVersion ? ' ' + c.harnessVersion : ''}` : null,
